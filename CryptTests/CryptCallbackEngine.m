...
- (OSStatus)getContextValue:(AuthorizationEngineRef)inEngine
  ...
  if ([self.context[key] isKindOfClass:[NSString class]]) {
    ...
    *outValue = value;
    ...
    NSData *data =
        [NSPropertyListSerialization dataWithPropertyList:(NSDictionary *)self.context[key]
                                                   format:NSPropertyListXMLFormat_v1_0
                                                  options:0
                                                    error:nil];
    value->length = (size_t)[data length];
    value->data = (void *)[data bytes];
    *outValue = value;
...
- (void)setUID:(uid_t)uid {
  ...
  self.context[@kAuthorizationEnvironmentUID] = data;
}

- (void)setGID:(gid_t)gid {
  NSData *data = [NSData dataWithBytes:&gid length:sizeof(gid_t)];
  self.context[@kAuthorizationEnvironmentGID] = data;
}
